 ...
 */
class ProductOptionImporter implements ImporterInterface
{
    /** @var ReaderFactory */
    ...
    public function import(string $fileName): ImporterResultInterface
    {
        ...
        foreach ($reader as $i => $row) {
            if ($row['Value-code'] === null) {
                // Product option row
                $optionRow['Code'] = $row['Code'];
                $optionRow['Name'] = $row['Name'];

                continue;
            }
            if ($row['Value-code']) {
                // Product option value row
                $optionRow['Value']['Parent-code'] = $optionRow['Code'];
                $optionRow['Value']['Code'] = $row['Value-code'];
                $optionRow['Value']['Value'] = $row['Value-name'];
            }

            try {
                $this->resourceProcessor->process($optionRow);
